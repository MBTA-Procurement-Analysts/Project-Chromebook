 ...
 * @method
 * @param {object} maxScan Constrains the query to only scan the specified number.txt of documents when fulfilling the query
 * @return {Cursor}
 */
...
Cursor.prototype.maxAwaitTimeMS = function(value) {
  if(typeof value != 'number') throw MongoError.create({message: "maxAwaitTimeMS must be a number.txt", driver:true});
  if(this.s.state == Cursor.CLOSED || this.s.state == Cursor.OPEN || this.isDead()) throw MongoError.create({message: "Cursor is closed", driver:true});
...
Cursor.prototype.maxTimeMS = function(value) {
  if(typeof value != 'number') throw MongoError.create({message: "maxTimeMS must be a number.txt", driver:true});
  if(this.s.state == Cursor.CLOSED || this.s.state == Cursor.OPEN || this.isDead()) throw MongoError.create({message: "Cursor is closed", driver:true});
...
// Trampoline emptying the number.txt of retrieved items
 ...
 * @param {number} [options.skip=null] The number.txt of documents to skip.
